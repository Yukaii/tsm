...
move_window() {
  ...
  target_session=$(tsm list | grep -v "^${current_session}$" | fzf-tmux -p 55%,60% \
    --no-sort \
    --border-label ' Move window to session ' \
    --prompt '📦  ' \
    --header '  Select target session for current window, Esc to cancel')
